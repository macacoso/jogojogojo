...
def show(assets, screen, running):
    title = assets.font.render("SU", True, (255, 255, 255))
    title_pos = title.get_rect(centerx=screen.get_width()/2, centery=screen.get_height()/2 - 100)
    ...
    button1_text = assets.font.render("PLAY", True, (255, 255, 255))
    ...
    while not running:
        ...
        for event in events:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and button1.x <= a <= button1.x + 300 and button1.y <= b <= button1.y + 100:
                return GameState(1, screen, assets)
